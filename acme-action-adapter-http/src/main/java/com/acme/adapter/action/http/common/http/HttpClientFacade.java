 ...
 */
package com.acme.adapter.action.http.common.http;

import com.acme.adapter.action.http.common.configuration.HttpFormsAdapterOptions;
import com.acme.adapter.action.http.common.configuration.HttpFormsSettings;
import com.acme.adapter.action.http.common.exception.UnsupportedFormsException;
import com.acme.adapter.action.http.common.placeholders.UriTransformer;
...
public class HttpClientFacade {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(
      com.acme.adapter.action.http.common.http.HttpClientFacade.class);
  ...
  private final List<HttpFormsSettings> services;

  private final WebClient webClient;

  private final CustomHttpHeader customHttpHeader;

  public HttpClientFacade(WebClient webClient, HttpFormsAdapterOptions configuration) {
  ...
  private String toUrl(Pair<ClientRequest, HttpFormsSettings> request) {
   ...
   * objects.
   */
  protected void validateContract(FormsAdapterRequest message) {
    if (message.getParams() == null || !message.getParams().containsKey(PATH_PROPERTY_KEY)) {
      throw new IllegalArgumentException("Parameter `path` was not defined in `params`!");
    }
  }
   ...
   * Request<br> - It must set path property of the request based on the params<br> - It might set
   * headers of the request if needed.<br>
   * <br>
   * In case of headers created modified in this method, ensure that your service configuration
   * allows passing those headers to the target service. See 'allowedRequestHeaders' section of the
   * configuration <br>
   *
  ...
  private Pair<ClientRequest, HttpFormsSettings> prepareRequestData(
    ...
    final Pair<ClientRequest, HttpFormsSettings> serviceData;
    ...
    final Optional<HttpFormsSettings> serviceMetadata = findServiceMetadata(
        serviceRequest.getPath());

    if (serviceMetadata.isPresent()) {
      final HttpFormsSettings metadata = serviceMetadata.get();
      ...
      final String error = String
          .format("No matching service definition for the requested path '%s'",
              serviceRequest.getPath());
      throw new UnsupportedFormsException(error);
    }
    return serviceData;
  }

  private Optional<HttpFormsSettings> findServiceMetadata(String servicePath) {
  ...
  private void updateRequestQueryParams(HttpRequest<Buffer> request, HttpFormsSettings metadata) {
